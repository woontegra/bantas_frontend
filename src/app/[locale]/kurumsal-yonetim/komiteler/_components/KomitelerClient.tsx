...
import {
  Users, ChevronDown, Layers, ShieldCheck, AlertTriangle, Info,
} from "lucide-react";

export interface Member    { name: string; role: string; }
export interface Committee { name: string; members: Member[]; }
export interface StatCard  { label: string; value: string; sublabel: string; }
export interface KomitelerData {
...
const STAT_ICONS  = [Layers, ShieldCheck, AlertTriangle];
const STAT_COLORS = [
...
export function KomitelerClient({ data }: { data: KomitelerData }) {
    ...
    <div className="min-w-0 flex-1 space-y-6">
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
        <div className="flex items-center gap-2 bg-gradient-to-r from-indigo-600 to-blue-600 px-6 py-4">
          ...
          <h2 className="text-lg font-semibold text-white">Yönetim Kurulu Komiteleri</h2>
        </div>
            ...
            <div key={i}>
              ...
              {expanded === i && (
                <div className="border-t border-gray-100 bg-gray-50 px-6 py-5">
                  <p className="mb-4 text-sm font-semibold text-indigo-600">Görev ve Çalışma Esasları</p>
                    ...
                    {c.members.map((m, mi) => (
                      ...
                      >
                        ...
                        <div>
                          ...
                          <p className="mt-0.5 text-sm text-indigo-600">{m.role}</p>
                        </div>
                      </div>
                    ))}
                  </div>
